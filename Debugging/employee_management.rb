...
# The issue occured on `line 70`. Since we need to access the getter method for `:serial_number`, it cannot exist in a private space. To fix the issue, we can either include `:serial_number` on `line 52` or create a protected space within the `Employee` class and place it there.
# In the latter fix, because the call to the getter method is happening on `line 60` and occurs within the `Employee` class, it is perfectly accessible.