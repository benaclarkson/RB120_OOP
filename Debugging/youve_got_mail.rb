...
# The issue here is that we are attempting to access the `send` method in the the `Mailing` module, but have not mixed it into the `PostalService` class. Adding the line `include Mailing` on `line 78` fixes this issue.