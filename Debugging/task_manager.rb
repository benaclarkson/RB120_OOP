...
# The issue here occurs on `line 39` when the new local variable `tasks` is created. This creates conflicting information between the `@tasks` instance variable and the local variable. Simply renaming the variable on `line 39` and `line 43` fixes the issue.
